...
const BULK_BATCH_SIZE = 500; // Open-Meteo supports up to 1000; use 500 for safety

// Open-Meteo requires literal commas in lat/lon lists — URLSearchParams encodes them as %2C which breaks bulk queries.
// We build the query string manually to preserve literal commas.
...
// Build a bulk Open-Meteo URL for multiple grid points at once
...
// Parse a bulk API response (array of location objects) into per-point time-series
...
// Generate grid lat/lon points across CONUS — bounds match projection calibration
...
export default function USHeatMap() {
  ...
  // Draw heatmap on canvas whenever data/timeIdx/colorScheme changes
  useEffect(() => {
    ...
    // Compute the pixel bounds of the entire grid region using the projection
    // then divide evenly — avoids sub-pixel drift at high resolutions
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      // Use integer pixel positions derived from grid indices — no floating-point drift
  ...
  const handleFetch = useCallback(async (config) => {
    ...
    if (config.useBulk) {
      // Bulk mode: send up to BULK_BATCH_SIZE locations per request
      // Open-Meteo returns an array of forecast objects when multiple lat/lon given
      for (let i = 0; i < total; i += BULK_BATCH_SIZE) {
        ...
        for (let attempt = 0; attempt < 2; attempt++) {
          try {
            ...
            // Multi-location: top-level array; single-location fallback: plain object
      ...
      // Single-location mode: one request per grid point, 8 at a time
      const CONCURRENCY = 8;
        ...
        <>
          ...
          >
            <div className="relative w-full" style={{ paddingBottom: '62.5%' }}>
              {/* Canvas for heatmap cells — absolute fill */}
              <canvas
                ref={canvasRef}
              ...
              {/* SVG for state borders + labels on top */}
              <svg
                ref={svgRef}
                viewBox={US_VIEWBOX}
              ...
              >
                {/* State abbreviation labels — positioned at centroids via projection */}
